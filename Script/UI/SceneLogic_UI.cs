...
namespace FrameWork.UI
{
    ...
    public partial class SceneLogic : MonoBehaviour
    {
        ...
        public bool isEscapeLock { get; set; } = false;


        //초기스폰위치        

        // PopPanel 후 호출. 패널 닫으면서 다른 패널 오픈하고자 할때 사용
        public UnityEvent FinishOpenPanelEvent = new UnityEvent();
        ...
        private void UI_Awake()
        {
            UI_Initialize();     // Panel, Popup 초기화
            UI_InitializeLate(); // Panel, Popup 등을 캐싱하기 위해 늦은 타이밍 이니셜라이즈
        }
        ...
        public Camera stackCamera { get; private set; }
        private void InitStackCamera()
        {
        ...
        private void SetStackCamera()
        {
            GameObject go = new GameObject(nameof(stackCamera));
            stackCamera = go.AddComponent<Camera>();
            stackCamera.cullingMask = 1 << LayerMask.NameToLayer("UI");
            canvas.worldCamera = stackCamera;

            UniversalAdditionalCameraData universalCamera = stackCamera.GetUniversalAdditionalCameraData();
            universalCamera.renderType = CameraRenderType.Overlay;
            universalCamera.renderPostProcessing = true;

            go.AddComponent<CameraShakeManager>();
        }
        ...
        private void UI_InitializeLate<T>(Dictionary<string, T> dic) where T : UIBase
        {
        ...
        /// 
        /// </summary>
        private void UI_InitializeLate()
        {
            UI_InitializeLate(_dicPanels);
            UI_InitializeLate(_dicPopups);
            UI_InitializeLate(_dicToasts);
        }
        #endregion
		...
		private void UI_Update()
		{
			if (Input.GetKeyDown(KeyCode.Escape))
			{
				...
				if (_stackPopups.Count <= 0 && isEscapeLock)
				{
					EscapeEvent?.Invoke();
        ...
        {
            if (_stackPanels.Count == 0) return null;
